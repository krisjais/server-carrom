...
const POINTS_TO_WIN = 25; // first to reach these match points also wins
...
function checkMatchWinner(match) {
  ...
  if ((match.scoreA || 0) >= POINTS_TO_WIN) return "A";
  if ((match.scoreB || 0) >= POINTS_TO_WIN) return "B";
  return null;
}
    ...
    const populated = await Promise.all(
      matches.map(async (match) => {
        ...
        if (m.teamAModel === "Team" && m.teamA) {
          m.teamA = await Team.findById(m.teamA._id || m.teamA).populate(
            "players",
          );
        }
        if (m.teamBModel === "Team" && m.teamB) {
          m.teamB = await Team.findById(m.teamB._id || m.teamB).populate(
            "players",
          );
        }
        return m;
...
router.put("/:id/board", async (req, res) => {
  try {
    ...
    const boardStarted = match.boardStartedAt
      ? new Date(match.boardStartedAt).getTime()
      : Date.now();
    ...
    const rawA = calcBoardScore(
      Number(coinsPocketedA),
      queenCoveredBy,
      "A",
      Number(foulsA),
    );
    const rawB = calcBoardScore(
      Number(coinsPocketedB),
      queenCoveredBy,
      "B",
      Number(foulsB),
    );
    ...
    // 5. Check match winner (boards/points)
    const matchWinner = checkMatchWinner(match);
    if (matchWinner) {
      match.status = "completed";
      match.winner = matchWinner === "A" ? match.teamA : match.teamB;
      match.winnerModel =
        matchWinner === "A" ? match.teamAModel : match.teamBModel;
      // Clear the board timer when match completes
      match.boardStartedAt = null;
    } else {
      // Reset board timer for the next board
      match.boardStartedAt = new Date();
    }

    // Clear any running board timer when match is completed
    if (match.status === "completed") match.boardStartedAt = null;
...
router.put("/:id/result", async (req, res) => {
  try {
    ...
    if (winner) {
      match.winner = winner;
      match.winnerModel =
        match.teamA && match.teamA.toString() === winner.toString()
          ? match.teamAModel
          : match.teamBModel;
    }
...
router.post("/advance", async (req, res) => {
  try {
    ...
    if (completedMatches.length === 0) {
      return res
        .status(400)
        .json({ error: "No completed matches in this round" });
    }
    ...
    // Single-player duplicate detection: ensure no player appears more than once
    if (matchType === "single") {
      const ids = winnerEntries.map((w) => (w.id || "").toString());
      const dupIds = ids.filter((v, i, a) => a.indexOf(v) !== i);
      const uniqueDupIds = [...new Set(dupIds)];
      if (uniqueDupIds.length > 0) {
        const dupPlayers = await Promise.all(
          uniqueDupIds.map(async (id) => {
            const player = await Player.findById(id);
            const matchesWithPlayer = completedMatches.filter(
              (m) => m.winner && m.winner.toString() === id,
            );
            return {
              id,
        ...
        return res.status(409).json({
          conflict: true,
          message: `Player conflict: ${dupPlayers.map((p) => p.name).join(", ")} appears multiple times among winners for round ${round}.`,
          duplicates: dupPlayers,
          round,
          matchType,
        });
      }
    ...
    if (
      (matchType === "double" || matchType === "mixed") &&
      winnerEntries.length >= 2
    ) {
      ...
      if (rank1?.players.length && rank2?.players.length) {
        ...
        const commonPlayers = rank2.players.filter((p) =>
          rank1Ids.includes(p._id.toString()),
        );

        if (commonPlayers.length > 0) {
          const rank3 = populatedWinners[2] || null;
          const rank3Team = rank3
            ? await Team.findById(rank3.id).populate("players")
            : null;
          ...
          return res.status(409).json({
            ...
            rank3Team: rank3Team
              ? {
                  ...
                  players: rank3Team.players.map((p) => ({
                    _id: p._id.toString(),
                    name: p.name,
                    gender: p.gender,
                  })),
    ...
    res.json({
      message: `Round ${nextRound} created with ${newMatches.length} matches`,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post("/resolve-conflict", async (req, res) => {
  try {
    const { matchType, round, rank2TeamId, removePlayerId, addPlayerId } =
      req.body;
    ...
    if (!rank2TeamId || !removePlayerId || !addPlayerId) {
      return res
        .status(400)
        .json({
          error: "rank2TeamId, removePlayerId, addPlayerId are required",
        });
    }
...
router.get("/rules", (_req, res) => {
  res.json({
    ...
    winCondition: `First to ${POINTS_TO_WIN} pts or first to win ${WIN_BOARDS} boards wins the match`,
    notes: [
